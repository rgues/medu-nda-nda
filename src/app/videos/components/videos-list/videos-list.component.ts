...
export class VideosListComponent implements OnInit {
  ...
  getVideos() {
    ...
    this.videos.getAllUnapprouveVideo(this.user.remenber_token).subscribe((videos: any) => {
      if (videos && videos.liste_videos) {
        this.defaultListVideos = videos.liste_videos;
  ...
  updateVideo(video: any) {
    this.videos.setVideosData(video.video);
    this.router.navigate(['/videos/update-video']);
  }
  ...
  approuveVideo(video: any) {
    ...
    this.videos.approuveVideo(member.remenber_token, param).subscribe(reponse => {

      if (reponse && reponse.message === 'success') {
        this.loadingShow = false;
        this.translate.get('VIDEO_APPROUVE_MSG').subscribe(trans => {
          this.toast.success(trans);
        });
        this.getVideos();
      }
