...
export class EventAttendanceComponent implements OnInit {
  ...
  // get the attendance member list
  getMembersAttendace() {
    this.eventService.getAttentanceState(this.token, this.eventId).subscribe(reponse => {
      if (reponse && reponse.members) {
        this.tempMember = reponse.members;
        const members: Members[] = [];
        this.tempMember.forEach(member => {
          members.push({
            choice: member.attend === 1 ? true : false,
  ...
  updateEventAttendance() {
    ...
    this.eventService.updateMemberAttendance(this.token, this.formEventAttendance.value).subscribe(reponse => {
 ...
 //  ===================================  Impression ================================
    ...
    imprimerPdf() {
      ...
      this.translate.get(['EVENT_LIST_PRESENCE','FORM_FIRSTNAME','FORM_LASTNAME'
      ,'PRINT_BY','PRINT_ON','PRINT_AT','FORM_EVENT'])
      .subscribe(trans => {
        this.translations.push(trans.EVENT_LIST_PRESENCE);
