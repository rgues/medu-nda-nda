 ...
 // public uploader: FileUploader = new FileUploader({ url: 'https://evening-anchorage-3159.herokuapp.com/api/' });
  ...
  initFormArticle() {
    this.formArticle = this.fb.group({
      ...
      auteur_id: ['', Validators.required],
      position_id: [''],
      categorie_id: [''],
      titre: ['', Validators.required],
      contenu_fr: ['', Validators.required],
      contenu_en: ['', Validators.required],
      expire: [0],
      date_expiration: [this.startDate],
  ...
  createArticle() {
    ...
    this.article.createArticle(member.remenber_token, this.formArticle.value).subscribe(reponse => {

      if (reponse && reponse.message === 'success') {
        this.loadingShow = false;
        this.translate.get('ARTILE_SUCCESS_MSG').subscribe(trans => {
