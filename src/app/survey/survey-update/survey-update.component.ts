...
export class SurveyUpdateComponent implements OnInit {
  ...
  initFormSurvey() {
    ...
    this.formSurvey = this.fb.group({
      survey_id : [surveys ? surveys[0].survey_id : ''],
      title_en: [dataEn ? dataEn.survey_title : '', Validators.required],
      description_en: [dataEn ? dataEn.survey_desc : '', Validators.required],
      title_fr: [dataFr ? dataFr.survey_title : '', Validators.required],
      description_fr: [dataFr ? dataFr.survey_desc : '', Validators.required]
   });
  }
  ...
  updateSurvey() {
    ...
    this.survey.updateSurvey(member.remenber_token, this.formSurvey.value).subscribe(reponse => {
      if (reponse && reponse.message === 'success') {
        this.loadingShow = false;
        this.translate.get('SURVEY_UPDATE_SUCCESS_MSG').subscribe(trans => {
